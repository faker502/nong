<template>
  <div class="page">
    <div class="part_1">
	  <nav-bar title="购买记录" right=""  @handleRight="$router.push({ path: '/score' })" titleColor='#fff' leftIconColor="#fff" class="nav-top"/>
    </div>
    <div class="container">
          ...
          <div v-if="active == 3">
            <van-list class="list-content" v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" :offset='50'>
              <div class="list_td"  v-for="it in billList" :key="it.id">
                <div class="w100">
                  <div class="text">购买金额：<span>{{ it.money }} 元</span></div>                  
                </div>
				<div class="w100">
                  <div class="text">购买时等级：{{ it.lvText }}</div>                  
                </div>
                <div class="w50">
                  <div class="text">日收益 {{ it.daily }} 元</div>
                  <div class="text">收益周期 {{ it.days }} 天</div>
                </div>
				<div class="w50">
				  <div class="text">总收益 {{ it.total }} 元</div>
				  <div class="text">{{ it.days }} 天后退回本金</div>
				</div>
