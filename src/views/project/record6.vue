<template>
  <div class="page">
    <div class="part_1">
	  <nav-bar title="申领记录" titleColor='#fff' leftIconColor="#fff" class="nav-top"/>
    </div>
    <div class="container">
...
"> {{ it.title }}</div>   
	  ...
	  <div class="text" style="
	  line-height: 35px;
	  "><van-button v-show="it.money > 0" @click="getPack(it.id, index)" round block type="default">{{ it.btn }}</van-button></div> 
			   
                </div>
				
                <div class="w50" v-if="it.money > 0">
                  <div class="text">
					  <div class="w40">
						  <div class="text"><img src="@/assets/photo/fund/aaa1.png" style="width:40px" /></div>
						  <div class="text">
							  <p>每季度发放</p>
							  <p>{{ it.money }}</p>
						  </div>
                  ...
                  <div class="text">
					  ...
					  <div class="w40">
							  <div class="text"><img src="@/assets/photo/fund/aaa2.png" style="width:40px" /></div>
							  <div class="text">
								  <p>保障周期</p>
								  <p>{{ it.remark }}</p>
							  </div>
...
import { myProjectApi6,getPackApi } from "@/api/order";
...
export default {
  ...
  methods: {
    ...
    getList() {
      let that = this
      myProjectApi6({ pageNum: that.pageNum }).then((res) => {
        if (res.code != 200) {
