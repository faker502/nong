<template>
  <div class="page">
    <div class="part_1">
	  <nav-bar title="申领记录" right=""  @handleRight="$router.push({ path: '/score' })" titleColor='#fff' leftIconColor="#fff" class="nav-top"/>
    </div>
    <div class="container">
          ...
          <div v-if="active == 3">
            <van-list class="list-content" v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" :offset='50'>
              <div class="list_td"  v-for="it in billList" :key="it.id">
                <div class="w100">
                  <div class="text">奖励金额：<span class="red">{{ it.money }} 元</span></div>                  
                </div>

				<div class="w50">
				  <div class="text">审核状态：<span>{{ it.lvText }}</span></div>
...
import { myProjectApi5 } from "@/api/order";
import { getAwardApi52 } from "@/api/project";
...
export default {
  ...
  methods: {
    ...
    getList() {
      let that = this
      myProjectApi5({ pageNum: that.pageNum }).then((res) => {
        if (res.code != 200) {
