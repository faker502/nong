          ...
          <div v-if="active == 3">
            <van-list class="list-content" v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" :offset='50'>
              <div class="list_td"  v-for="(it, index) in billList" :key="index">
				...
				<div class="w80">
				  <div class="text">周期：<span>{{ it.days }}</span></div>
				</div>
				<!--
				<div class="w80">
				  <div class="text">收益：<span>{{ it.daily }}</span></div>
				</div>
				...
				<div class="w80" v-show="it.modeTitle">
				  <div class="text">{{ it.modeTitle }}<span>{{ it.modeDesc }}</span></div>
				</div>
				...
				<div class="w100" v-show="it.more">
					<div class="text" v-html="it.more"></div>
				</div>
...
import { myProjectMsApi2,getPackApi } from "@/api/fund";
...
export default {
  ...
  methods: {
    ...
    getList() {
      let that = this
      myProjectMsApi2({ pageNum: that.pageNum }).then((res) => {
        if (res.code != 200) {
