<template>
  <div class="page">
    <div class="part_1">
	  <nav-bar title="兑换记录" right=""  @handleRight="$router.push({ path: '/score' })" titleColor='#fff' leftIconColor="#fff" class="nav-top"/>
    </div>
    <div class="container">
          ...
          <div v-if="active == 3">
            <van-list class="list-content" v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" :offset='50'>
              <div class="list_td"  v-for="it in billList" :key="it.id">
				...
				<div class="w50">
				 <div class="text" style="font-size:12px;color: rgba(110, 102, 102, 1);">{{ it.createTime }}</div>
				  <div class="text" style="text-align:right;"><span style="color: rgba(112, 120, 61, 1);font-size:14px">{{ it.lvText }}</span></div>
<!-- 				  <div class="text" style="text-align:right;" v-show="it.status == 2" @click="getAward(it.id)">
					  
					  <van-button  round block type="default">重新申请</van-button>
					  
				  </div> -->
				</div>  
				  
                <div class="w100" v-show="it.remark">
                  <div class="text"><span>{{ it.remark }}</span></div>                  
                </div>
				<div class="w100" v-show="it.money != '0.00'">
                  <div class="text"><span>兑换现金{{ it.money }}元</span></div>                  
                </div>
				<div class="w100" v-show="it.fund == '0.00'">
                  <div class="text">积分 -{{ it.num }} </div>                  
                </div>
				<div class="w100" v-show="it.fund != '0.00'">
                  <div class="text">专项基金：{{ it.fund }} 元</div>                  
                </div>

				<div class="w100" v-show="it.addr">
				  <div class="text">收货地址：{{ it.addr }}</div>                  
				</div>

				<div class="w100" v-show="it.status == 8">
				  <div class="text" v-show="it.other">拒绝原因：<span>{{ it.other }}</span></div>                  
				  <!-- <div class="text" style="font-weight:600;">请联系APP在线客服</div>    -->              
				</div>
...
import { getAwardApi4,myProjectApi4 } from "@/api/project";
...
export default {
  ...
  methods: {
    ...
    getList() {
      let that = this
      myProjectApi4({ pageNum: that.pageNum }).then((res) => {
        if (res.code != 200) {
