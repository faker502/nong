    ...
    <div class="test-page">
        ...
        <div class="top">
			...
			<div class="top-head">
				...
				<div class="right-m0" @click="$router.push({ path: '/fund/record2'})">领取记录</div>
...
<!-- 				<div class="tabs data_tabs2 df_r">
					...
					<div class="data_tab2" @click="$router.push('/fund-in')">
					    预审基金
					</div>
			...
			<div class="top-head-ch" style="background:#fff;padding-top:10px;position:relative;    border-top-left-radius: 6px;
    border-top-right-radius: 6px;">	
				<div class="lq" @click="goFund()">
					
					<img src="@/assets/photo/fund.webp?1" style="width:100%;height:250px;" />
					
				</div>
				<div style="position:absolute;height:35px;font-size:21px;font-weight:600;text-align:right;top:220px;right:20px;">
					{{ fund }}
				</div>


				<div class="lq" @click="$router.push('/pro-list6')">
					
					<img src="@/assets/photo/fund/ban2.webp?1" style="width:100%;height:200px;" />
					
				</div>
				
				<div class="lq" @click="$router.push('/lva')">
					
					<img src="@/assets/photo/fund/ban3.webp?1" style="width:100%;height:200px;" />
...
<!--                        <div class="header">
                        ...
                        <div class="body">
							...
							<div class="f-1">
								<img src="@/assets/photo/user.webp" style="height:40px;padding-top:10px;" />
							</div>	
							<div class="f-1">
								
								
								<div style="display:block;line-height:25px;height:25px;text-indent:8px;">
									{{ item.title }}
								</div>	
								
								<div style="display:block;line-height:25px;height:25px;padding-top: 5px;">
									<div class="proc" style="padding-top: 8px; width: 90%;">
										<van-progress
										  stroke-width="8" 
								...
								<div style="display:block;line-height:25px;height:25px;text-indent:8px;padding-right:20px;padding-top:7px;" v-show="item.award">
									<div style="display:grid;grid-template-columns:auto 80px;">
										<div>{{ item.tip }}</div>
										
										
										<div @click="getAward(item.num, item.progress)" >
											<van-button  round block type="default" :class="item.progress < 100 ? 'van-button-gray' : ''" >{{ item.btn }}</van-button>
										</div>									
...
import { getListApi, getDetailApi, buyProjectApi, getAwardApi } from "@/api/project";
...
export default {
    ...
    methods: {
		getAward(num, progress) {
			if (parseInt(progress) < 100) {
				Toast('邀请人数不足');
				return;
			}
			getAwardApi({'num':num}).then((res) => {
				Toast(res.msg);	
				if (res.code == 200) {
					this.getProjectList();
				}			
						...
						.f-1 {
							...
							.unit{
							 ...
							 .van-button {
								 ...
								 font-size: 13px;
								 color:#fff;
								 background:linear-gradient(180deg, #c0c79e 0%, #c0c79e 100%);
							 }
							 .van-button-gray {
								 height: 27px;
								 font-size: 13px;
								 background: linear-gradient(180deg,#e9e9e9,#ddd) !important;
								  color:#fff;
							 }
