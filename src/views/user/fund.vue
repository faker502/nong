<template>
  <div class="page">
    ...
    <div class="container">
      ...
      <div class="wrap">
        <div class="list">
          <div class="fund-total detail-money" v-if="active == 3">
			...
			<div class="detail-1">
			  <div class="detail-2">
				  ...
				  <div class="flex-item">
					  ...
					  <van-button style="margin-left: 15px;" @click="$router.push({ path: '/transfer/search' });" type="default">转账</van-button>
					  </div>
				  </div>
			</div>
			
			
			<div class="detail-2" v-show="account.online == 'yes'">
				  <div class="flex-item">
					  <p class="text">充值余额:</p>
					  <p class="gold">{{ account.recharge_over }}</p>
				  </div>
				  <div class="flex-item">
					  <div class="v-btn">
						  <van-button @click="$router.push({ path: '/recharge' });" type="default">充值</van-button>
					  <van-button style="margin-left: 15px;" @click="$router.push({ path: '/transfer/search' });" type="default">转账</van-button>
					  </div>
				  </div>
			</div>
          ...
          <div class="fund-recharge detail-money" v-else-if="active == 4">
			  ...
			  <div class="detail-1">
			    <div class="detail-2">
				  ...
				  <div class="flex-item">
					  <div class="v-btn" v-show="account.online == 'yes'">
					  <van-button @click="$router.push({ path: '/transfer/search' });" type="default">转账</van-button>
					  </div>
				  </div>
			  </div>
			  </div>
          ...
          <div class="detail-money" v-if="active == 2">
			  
			  <div class="detail-1">
			    <div class="detail-2">
			  	  <div class="flex-item">
			  		  <p class="text">数字人民币余额:</p>
			  		  <p class="gold">{{ account.rmb }}</p>
			  	  </div>
			  </div>
			  </div>
          ...
          <div v-else>
            ...
            <van-list class="list-content" v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" :offset='50' v-if="active != 1">
              <div class="list_td" v-for="it in billList" :key="it.id">
                <div class="left">
                  <div class="item-1" style="line-height: 24px;width: 125%;">{{ it.remark }}</div>
                  <div class="item-1" style="line-height: 24px;">{{ it.create_time }}</div>
			...
			<van-list class="list-content" v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" :offset='50' v-else>
			  <div class="list_td" v-for="it in billList" :key="it.id">
			    <div class="left">
			      <div class="item-1" style="line-height: 24px;width: 125%;">{{ it.remark }}</div>
			      <div class="item-1" style="line-height: 24px;">{{ it.create_time }}</div>
...
<script>
import { getBillApi, getAccountApi, getAccountReceiveApi } from "@/api/member";
export default {
  data() {
    return {
      tabs: [
