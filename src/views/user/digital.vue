  ...
  <div class="page">
    ...
    <div class="container">
      ...
      <div class="wrap">
        ...
        <div class="list">
          <div class="fund-total" v-if="active == 2">
          ...
          <div v-if="active == 2">
            <van-list class="list-content" v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" :offset='50'>
              <div class="list_td"  v-for="it in billList" :key="it.id">
                <div class="left">
                  <div class="item-1" style="line-height: 24px;width: 125%;">{{ it.name }} </div>
                  <div class="item-1" style="line-height: 24px;width: 125%;">{{ it.phone }}</div>
                </div>
                <div class="right">
                  {{ it.update_time }}
                </div>
...
<script>
import { getInviteApi, getAccountApi } from "@/api/member";
export default {
  data() {
    return {
      title: '我的推荐',
  ...
  methods: {
    ...
    getList() {
      let that = this
      getInviteApi({ type: parseInt(this.active), pageNum: that.pageNum }).then((res) => {
        if (res.code != 200) {
