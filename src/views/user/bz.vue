<template>
    <div class="test-page">
		...
		<div class="part_1">
		  <nav-bar title="保障专项补助金" right=""   titleColor='#fff' leftIconColor="#fff" class="nav-top"/>
		</div>
        ...
        <div class="top">
			...
			<div class="top-head-ch" style="background:#fff;padding-top:0;text-align:center;">	
				<div class="lq">
					...
					专项基金200万元 -->
					<img src="@/assets/photo/bz.webp"  @click="buyProject" style="width:95%;margin:10px auto 0 auto;border-radius: 6px;"  />
				</div>
...
<!-- 				<div class="lq3">
				...
				<div style="margin: 5px; padding-bottom: 10px;" >
				  ...
				  <div style="line-height: 200%;text-align: left;width:90%;margin: 0 auto;position: relative;display: grid;grid-template-columns: 50% 50%;">
					  
					  <div style="display:grid;grid-template-columns: 55px auto;">
						  <div style="font-size:12px;color: rgba(14, 37, 41, 1);display: flex;">
						  	<img src="@/assets/photo/bz1.webp" style="width:48px;"  />
						  </div>
					  		<div style="display: flex;flex-direction: column;">
								<div style="font-size:12px;color: rgba(14, 37, 41, 1); display: flex;">总余额</div>
								<div style="font-size:18px;font-weight:600;display: flex;padding-top: 5px;">{{ acc.bz }}</div>
							</div>				 
					  </div>
					  
					  <div style="display:grid;grid-template-columns: 55px auto;">
										<div style="font-size:12px;color: rgba(14, 37, 41, 1);display: flex;">
											<img src="@/assets/photo/bz2.webp" style="width:48px;"  />
										</div>
										<div style="display: flex; flex-direction: column;">
					  						  <div style="font-size:12px;color: rgba(14, 37, 41, 1);display: flex;">待提现</div>
					  						  <div style="font-size:18px;font-weight:600;display: flex;padding-top: 5px;">{{ acc.bzo }}</div>
        ...
        <div class="page1">
            ...
            <div class="content">
                <div class="content-box">
					...
					<div class="lq" style="height: auto; background-color: #fff; min-height: 100px;">
						<img src="@/assets/photo/bzb.webp" style="width:100%;border-top-right-radius: 6px;border-top-left-radius: 6px;"/>	
						
						<div style="width:95%;text-align: center;display: block;font-size: 14px;line-height: 32px;" v-show="bzList.length == 0">暂无领取记录</div>
						
						<div style="width:95%;text-align: center;display: block;font-size: 14px;line-height: 32px;" v-for="(it, index) in bzList" :key="it.id" v-html="it.desc">
					...
					<div class="lq" >
						<img src="@/assets/photo/bzn.webp" style="width:100%;border-top-right-radius: 6px;border-top-left-radius: 6px;"/>	
					</div>
       ...
       <van-form ref="pwdForm" v-show="pwdShow" class="mt15 passwdForm" @submit="submitPwd">
		   <div style="margin:15px 10px 0 30px;font-size: 16px;font-weight:600" >输入支付密码</div>
...
import { getListApi, getDetailApi, buyProject222Api, getAwardApi333,getAward333,getJFAwardApi } from "@/api/project";
        ...
        submitPwd() {
            ...
            if (this.password.length == 6) {
                if (this.isSubmit) {
                    ...
                    //this.loadingText = "";
                    this.isSubmit = false;
                    buyProject222Api({ fund:this.fund, pay_password: this.password }).then((res) => {
                        this.isSubmit = true;
                        this.loading = false;
                        if (res.code == 200) {
                            //传入对象参数
