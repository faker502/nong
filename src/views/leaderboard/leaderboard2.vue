<template>
  <div class="page">
	...
	<nav-bar title="预审排行榜" titleColor="#fff" leftIconColor="#fff" :isLeftFn="true" @handleBack="$router.push('/')" />
...
<!-- 	  <div class="tabs data_tabs2 df_r">
	  	...
	  	<div class="data_tab2"  @click="$router.push('/rank')">
	  	    全民复兴排行
	  	</div>
...
<script>
import { getTeamRankingApi2, getRankingApi, getTodayRankingApi2, getYesterdayTopApi2 } from "@/api/member";
...
export default {
  ...
  methods: {
    ...
    getTodayRankingApi() { 
      let that = this;
      getTodayRankingApi2({ pageNum: that.pageNum }).then((res) => {
    ...
    async getTeamRanking() {
      let that = this;
      getTeamRankingApi2({ pageNum: that.pageNum }).then((res) => {
        if (that.pageNum == 1) {
