<template>
  <div class="page">
    ...
    <div class="part_2">
		...
		<nav-bar title="收货地址" titleColor='#fff' leftIconColor="#fff" class="nav-top"/>
...
<!-- 		  <div class="title">
	...
	<select v-model="prov" class="picker">
		<option v-for="option in arr" :value="option.name">
			{{ option.name }}
		</option>
	</select>
	<select v-model="city" class="picker">
		<option v-for="option in cityArr" :value="option.name">
			{{ option.name }}
		</option>
	</select>
	<select v-model="area" v-if="area" class="picker">		
		<option v-for="option in areaArr" :value="option.name">
			{{ option.name }}
		</option>
	</select>
	  ...
	  ></van-field>
		...
		<div class="btns df_r">
		  <van-button
		    ...
		    @click="handleSubmit"
		     style="color:#A7AF78;border: 1px solid #b0be6b;background: rgb(242, 243, 230);">保存</van-button>
		</div>
      </van-form>
...
import { getAddressApi,setAddressApi } from "@/api/member";
...
mounted() {
  ...
  methods: {
	  handleSubmit() {		 		
	    ...
	    setAddressApi({name:this.form.name,phone:this.form.phone,street:this.form.street,prov:this.prov,city:this.city,area:this.area}).then((res) => {
	      if (res.code == 200) {
			  Toast(res.msg);
	 ...
	 getAddress() { 
	  getAddressApi().then((res) => {
	    ...
	    updateDistrict: function (flag) {
	      ...
	      if (this.areaArr && this.areaArr.length > 0 && this.areaArr[1].name) {
			  if (this.form.area) {
