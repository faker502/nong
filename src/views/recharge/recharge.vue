<template>
  <div class="page">
	...
	<nav-bar title="在线充值" titleColor='#fff' leftIconColor="#fff" right='充值记录' class="nav-top"  @handleRight="$router.push('/record_recharge')"/>
    <div class="part_1">
		
    </div>
    <div class="part_2">
		
		<div class="info_box" style="display: none;border-radius: 5px;width:auto; margin:0 auto 10px auto; padding: 10px 20px;">
		  <div class="my-inviter">
		    <p style="font-size: 12px;color:#626571;font-weight: 400;padding: 10px 0;">账户余额</p>
		    <p style="margin: 0 10px 0 0;font-size: 18px;color:#D11A2D;font-weight: 600;">
		      1000 <span style="color:#3F3D38;font-size:12px;font-weight: 400;">CNY</span>
		    </p>
		  </div>
		</div>
      ...
      <div class="method_box" v-else>
        ...
        <div class="m_item">
          ...
          />
                ...
                <div class="m_title" style="margin: 25px 0 25px;">请选择<span style="color:rgb(137 153 60)"> {{ payName }} </span>充值金额</div>

        <!--<div class="money-select" v-for="(it, k) in payChannelList" :key="k" :class="k" v-show="payType == k">-->
        <div class="money-select" v-for="(it, k) in payChannelList" :key="k" :class="k" v-show="payType == k">
        
          <div class="no-have" v-if="payChannelList[k].length == 0" style="background: #F3EDED;text-align: center;border: 1px solid #E44E49;border-radius: 3px;padding: 12px 0;">{{ payName }}暂时不可用</div>
          <div class="money-grid" v-else>
            ...
            <div
              ...
              @click="moneyOn(item)" :id="nameId(item)" v-if="item.indexOf('-') == -1"
            > 
              {{ item }} 元 
            </div>
            <div class="between" v-for="(item, key) in payChannelList[k]"
              :key="key"
              :class="[money == item ? 'money-on' : '']"
              @click="moneyOn(item)" :id="nameId(item)" v-if="item.indexOf('-') != -1">
              {{ item }} 
        ...
        <div class="select_money">
            <div v-if="moneyType == 0" class="money-text" style="margin-top: 30px;margin-left:10px">
              <span style="padding-right:4px">当前充值金额：</span>
              <span style="color:red">¥ {{ money }}</span> 
            </div>
            <div v-else class="money-input" style="margin-top: 30px;margin-left:10px">
              <span style="padding-right:4px">当前充值金额：</span>
              <van-field v-model="moneySelect" label="¥" placeholder="请输入充值金额" />
            </div>
        </div>
      </div>
      <div class="btns df_c">
...
export default {
  ...
  methods: {
    ...
    requestApi() { 
	  ...
	  if (this.moneySelect == undefined || this.moneySelect == '' || this.moneySelect <= 0) {
	  ...
	  if (this.online == 'yes') {
	    ...
	    if (moneyLen > 0) {
	      ...
	      if ((parseFloat(this.moneySelect) > parseFloat(endMoney)) || (parseFloat(this.moneySelect) < parseFloat(startMoney))) {
	        this.$toast({
	    ...
	    this.isSubmit = true;
	    payRequestApi({ 'payID': this.payType, 'money': this.moneySelect, 'channelSelect': this.channelSelect }).then((res) => {
	      this.isSubmit = false;
	      //console.log(res);
...
.page {
  ...
  .part_2 {
    ...
    .btns {
	  ...
	  .mb_20 {
		  background: linear-gradient(180deg, #E9E9E9 0%, #DDDDDD 100%) !important;
	  }
    }
  }
}
